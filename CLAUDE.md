...
| `JennPolytopeIndividual` | polytope-params object | ThreeD | ("Jenn Polytope 4D" in the UI.) Reimplements the *look* of Fritz Obermeyer's Jenn3d (jenn3d.org) — the Cayley-graph visualiser Nicolau & Costelloe drove with GE (EvoMUSART 2014). Precomputed 4D vertex tables for the regular polychora (5/8/16/24-cell + 600-cell; edges recovered by nearest-neighbour on S³, validated against known edge counts in the tests), evolved SO(4) rotation (6 plane angles), stereographic S³→R³ projection (curved edges *and* curved surfaces), tube thickness, `renderStyle` (wire rods / solid faces / both), vertex markers, reversible palette (w-depth → hue). Edges are sampled along their great-circle arc (SLERP) and swept into triangle tubes; nodes are icosahedral balls (a bit wider than the rods); 2-faces (recovered from the edge graph — triangles, or squares for the tesseract) are meshed as curved patches (barycentric/bilinear grid re-normalised onto S³ then projected). Renders as a **two-material `THREE.Group`**: opaque rods+balls seen *through* strongly-transparent (`opacity` ~0.26, `depthWrite:false`) glass faces — the Jenn look. `visualize()` builds the group directly (not via `ThreeDModality`) and hands it to `addMeshToScene`/`renderMeshToCanvas`; `generate3DPoints()` merges both buckets into one mesh for STL/tests. Arc sampling and face subdivision are gated by *projected* size/bulge (sagitta); a `lod` factor (0.5 for the 128px grid tiles, 1 for the 768px zoom + STL) keeps the 600-cell tile ~40k tris instead of ~120k. Gets STL export free. **Animated 4D rotation**: the zoom lightbox rotates the polytope in the w-planes over time (`animatesGeometry()`/`setAnimationTime()` hooks — the zoom loop rebuilds the mesh each frame at capped LOD, `resetAnimation()` on close), which is also what makes the curved faces read as curved (it sweeps them through the high-curvature near-pole zone; a static face's curvature is partly in the depth axis). Grid tiles stay static (per-frame CPU rebuild ×16 is too heavy). Genome = small PTO param vector (in the spirit of the "64 bits" paper); geometry built in the individual (not the generator), like the tree/DAG types. **Deferred: the 120-cell + true truncations/omnitruncations (need a Wythoff/reflection engine).** |
