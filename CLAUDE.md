...
| `JennPolytopeIndividual` | polytope-params object | ThreeD | ("Jenn Polytope 4D" in the UI.) Reimplements the *look* of Fritz Obermeyer's Jenn3d (jenn3d.org) — the Cayley-graph visualiser Nicolau & Costelloe drove with GE (EvoMUSART 2014). Precomputed 4D vertex tables for the regular polychora (5/8/16/24-cell + 600-cell; edges recovered by nearest-neighbour on S³, validated against known edge counts in the tests), **plus two families that give the concentric-ring / zig-zag ("antiprismatic") stereographic look the regular set can't make** (the Nicolau & Costelloe figures): the semiregular **grand antiprism** (carved from the 600-cell by removing two completely-orthogonal decagonal rings — 100 verts / 500 edges, found via the great-circle recurrence `pₖ₊₁ = φ·pₖ − pₖ₋₁`, validated in the tests; then **canonically re-oriented** so the two removed decagons' planes are the coordinate 2-planes — carving leaves an arbitrary pose, but its zig-zag antiprism bands only read cleanly down this axis, so the natural/near-identity pose and the w-plane 4D-morph sweep now show them rather than the generic 20-antiprism + 300-tetrahedron tangle; **and its rotation genome is biased near that axis** — the three w-plane angles xw/yw/zw, the ones that rotate vertices through the projection pole and tangle it, are capped to `JENN_GA_WCAP`≈0.3 rad, which PTO 'fine' mutation's clamp then keeps near-axis for the whole lineage, while the pole-preserving xy/xz/yz planes stay full-range for orientation variety and the zoom morph still sweeps w dynamically) and the parametric **duoprisms** `{p}×{q}` (Cartesian product of a p-gon and q-gon on the Clifford torus; two concentric rings joined by a lattice — carrying **explicit** edges/faces since for p≠q the two edge families differ in length, so nearest-neighbour recovery would drop the longer one). The duoprism's `p,q` are conditional genome genes (3–12, PTO structural naming handles the dependent search space, as Robot does). Evolved SO(4) rotation (6 plane angles), stereographic S³→R³ projection (curved edges *and* curved surfaces) with a soft `tanh` far-field radial bound `maxProjRadius`=6 (see below), tube thickness, `renderStyle` (wire rods / solid = faces + rods / both), reversible palette (w-depth → hue). (Rods draw in every non-wire mode so the curved edges stay crisp over the glass faces.) Edges are sampled along their great-circle arc (SLERP) and swept into triangle tubes using a **rotation-minimizing (parallel-transport) frame** (a per-ring fixed-up frame flipped orientation where the tangent crossed a switch threshold, pinching the low-poly tube — the "thinning"); **ball nodes are drawn at every vertex, always** (3.4× rod radius — since stereographic projection is *conformal* it preserves the polytope's real vertex corners, which can't be smoothed away, only capped by a ball, à la Jenn); 2-faces (recovered from the edge graph — triangles, or squares for the tesseract; duoprisms supply their square faces explicitly) are meshed as curved patches (barycentric/bilinear grid re-normalised onto S³ then projected). Renders as a **two-material `THREE.Group`**: opaque rods+balls seen *through* strongly-transparent (`opacity` 0.24, `depthWrite:false`) glass faces — the edge-dominant Jenn look (low opacity so the crisp rods+nodes carry the structure and a face caught edge-on recedes). `visualize()` builds the group directly (not via `ThreeDModality`) and hands it to `addMeshToScene`/`renderMeshToCanvas`; `generate3DPoints()` merges both buckets into one mesh for STL/tests. Rod arc sampling is gated by *projected* arc length; a `lod` factor (0.5 for the 128px grid tiles, 1 for the 768px zoom + STL) keeps tri counts down. **Face subdivision has two modes:** static (paused zoom / tiles / STL) uses a per-face projected-arc-length depth (cap 40); while 4D-**morphing** it uses a *rotation-invariant* budgeted uniform depth (`_faceDepthStable` — ~60k tris/frame split evenly across the shape's faces). The uniform-during-morph rule is essential: a face's **limb** (silhouette, which cuts *across* the face) is a polygon whose corner count otherwise pops as the projection changes ("the number of angles keeps changing"), and only a rotation-invariant depth is flicker-free — so spend a fixed budget to make it as fine as possible rather than varying it per frame. **`maxProjRadius` (tanh far-field bound):** stereographic projection flings a near-pole vertex toward infinity, dragging its faces into large flat "panels"; the bound caps their reach (6, down from 12) to shrink those panels toward the core — a projection-singularity artifact — while barely touching the core/mid loops that sit well inside it. Smooth `tanh` (not a hard clamp) so rods sweeping outward stay curved. **Robust camera framing:** `visualize()` publishes `group.userData.framingCenter/framingRadius` (median centre + interpolated ~80th-pctile vertex radius, ignoring near-pole outliers) and `renderMeshToCanvas` frames on that when present, so the core fills the view — a sensible default zoom, *not* a fix for the panels (`[`/`]` zoom-out still shows them; the real levers are `maxProjRadius` + subdivision + opacity). Gets STL export free. **Animated 4D rotation**: the zoom lightbox rotates the polytope in the w-planes over time (`animatesGeometry()`/`setAnimationTime()` hooks — the zoom loop rebuilds the mesh each frame at capped LOD, `resetAnimation()` on close), which is also what makes the curved faces read as curved (it sweeps them through the high-curvature near-pole zone; a static face's curvature is partly in the depth axis). Grid tiles stay static (per-frame CPU rebuild ×16 is too heavy). Genome = small PTO param vector (in the spirit of the "64 bits" paper); geometry built in the individual (not the generator), like the tree/DAG types. **Deferred: the 120-cell + true truncations/omnitruncations (need a Wythoff/reflection engine).** (The grand antiprism sidesteps that engine — it's a *subset* of the 600-cell — and the duoprisms are elementary products, so both land without Wythoff.) A headless dev tool renders any shape's edge-only stereographic projection to a PNG for QA: `node scripts/jenn-preview.js <shape|duoprism:PxQ> [out.png]` (reuses the real `jennGeometry` via the test harness + the shared `scripts/lib/png.js` encoder). |
| `EndlessFormsIndividual` | plain-data CPPN (DAG) | ThreeD | ("Endless Forms" in the UI.) Homage to EndlessForms.com (Clune, Yosinski, Lipson et al., Cornell): each voxel of a cubic grid is turned ON/OFF by one **CPPN** (Compositional Pattern Producing Network) queried at the voxel's coordinates. Inputs are `x,y,z`, `d` (radial distance) and a bias; nodes carry heterogeneous activations (`sin/cos/gauss/sigmoid/tanh/abs/identity/square`) with weighted inputs from earlier nodes (acyclic) — a `symX` gene folds `x→|x|` for the bilateral symmetry common in the gallery. Same plain-data / index-based / acyclic DAG shape as `PhotoFilterIndividual` and evaluated the same way — a **pure stateless memoised walk** in the individual, NOT `buildDAG` (so no `DAGRepresentation.js`). Coordinates are fed at a spatial frequency (an evolvable `coordScale` **gene**, fine Gaussian creep, range `EF_COORD_SCALE_RANGE`; `EF_COORD_SCALE` is the fallback) so periodic activations ripple into structure rather than a dull half-space slab — low freq ≈ big smooth blobs, high ≈ fine ridged detail. Two render modes off the same cached signed scalar field (`_field()`): `EF_SMOOTH` true = a **smooth iso-surface via Naive Surface Nets** (`_smoothMesh` — the dual of marching cubes: one shared vertex per straddling cell at the mean of its edge crossings, so `computeVertexNormals()` gives smooth non-faceted shading — the EndlessForms look; compact + table-free); false = **blocky "Minecraft" voxels** (`_blockyMesh`, exposed-face culling). Grid is non-cubic: `EF_GRID` (x/z) × `EF_GRID_Y` (taller, matching the gallery's portrait forms), with the query y-range scaled by `EF_GRID_Y/EF_GRID` so voxels stay cubic. Rides the shared Three.js pipeline (STL export free); height drives the palette. `validate()` rejects empty/near-solid forms so the grid shows sculptural shapes. Headless QA: `node scripts/endlessforms-preview.js [out.png] [--n 9]` renders the real `generate3DPoints()` mesh to a PNG (z-buffered, smooth per-vertex-normal shading — mirrors the app; mirrors `jenn-preview.js`/`sit-preview.js`). |
...
| `StructuralInformationIndividual` | plain-data I/S/A code tree | Canvas2D | ("Structural Information" in the UI.) A *generative* reading of Leeuwenberg's **Structural Information Theory** — normally an analysis theory (figure → shortest code = the Prägnant percept), run backwards: generate codes, decode to figures, so the minimum principle becomes a *prior baked into the representation* (short codes ⇒ regular/symmetric/repetitive figures, so random codes already look like ornaments). Code = a tree of turtle primitives `{turn,len,draw}` under the three holographic regularities — **iteration** `n*(c)` (chained; or **anchored** via internal turtle push/pop, replicating the chunk about a fixed reference point decoupled from its own displacement — a **rosette** `n⟳θ(c)` when the between-copy transform is a rotation, or a **row/grid** `n⇉d(c)` when it's a pen-up translation — the plane isometries), **symmetry** (`S[c]` = *point/2-fold rotational* via reverse-and-negate; `M[c]` = true **bilateral mirror** via a handedness flip flag), **alternation** `A[k:s…]` (a constant chunk distributed over a series). Turtle state carries a push/pop stack + a ±1 handedness (flip → reflection). This covers SIT's full isometry vocabulary (translation/rotation/reflection/glide); the anchored operators are the (b)-style "operator with a reference point" that fixes the open-motif-about-a-centre case a pure contour turtle can't. Still deferred: *emergent* salient points (intersections/centroids) — not part of SIT. Each operator generates its child *once* and replicates it, so the regularity is genuinely reused and heritable (what makes it SIT, not random mirrors). Metric 45° alphabet (turns multiples of 45°, lengths {1,√2,2} — √2 so diagonals close). Each primitive has a three-way `mode`: **stroke** (draw a segment), **dot** (stamp a point — SIT's `0` point primitive, for dot patterns), or **move** (pen-up). Multi-part figures come from pen-up moves, one traversal visiting every part — SIT needs no branching. The **continuous** sibling `StructuralInformationContinuousIndividual` (subclass overriding only `defaultRepresentation()`) swaps the discrete alphabet for continuous uniform turn/length — an A/B on discrete-vs-continuous primitives (crisp closure vs organic curves/smooth Gaussian-creep evolution). `describeExtra()` reports the **structural information load** (compact-tree primitives+operators → expanded steps, with compression ratio) and the SIT-notation code string. Companion to the turtle types (`Anemone`/`Branch`/`LSystem`), and the *subset* predecessor of the `SITCode*` pair below. |
| `SITCodeIndividual` | plain-data Leeuwenberg code tree + `family` | Canvas2D | ("Leeuwenberg Code 2D" in the UI.) The **full 1971 language** of Leeuwenberg's *A Perceptual Coding Language for Visual and Auditory Patterns*, where `StructuralInformationIndividual` above implements only the later three-regularity subset. The engine is `SITLanguage.js` (app-level, stateless, like `Palette`/`Photo`); the individual supplies only the generator + rendering. **All 4 information units** (`n` number, `∫` integration = running totals, so relative→absolute angles, `R` reversal = sequence then its reverse, `±` left-right variation), **all 5 operations** (`+`, `×`, `·` iteration, `*` vector addition, `⊛` addition of coincident angles) plus **one-sided iteration** (`n;`/`nᐟ` — n elements of one operand per 1 of the other), **all the indicators** (`⦃ ⦄` continuation, `{ }` chunking, `( )` border, `[ ]`/`⟦ ⟧` breakdown, `/ /` reprisal, `\| \|` absolute angles, `‖` independence of angles, `⟨ ⟩` outerproduct, `‾` vanishing sign), and the **combinatory/serial/parallel rules**. The paper's key reduction is that a figure is a stream of **angles**, one per elementary *grain* length — so length is never a primitive: a straight edge is `n·(0)`. `parallel structure` is a push/pop branching turtle ("used at nodes, from which different branches derive"); the vanishing sign is what yields dot patterns and multi-part figures from one contour trace. **Discrete and continuous are unified in one gene**: a code carries a rotational family N ∈ 3…360, every angle literal is an integer multiple of 360/N, and the multiple is capped — so small N is coarse/on-grid/closing, large N is fine *and* gentle (N=360 turns ≤8°, i.e. smooth curves). That replaces the discrete/continuous *pair* the older type needed. Deliberate simplifications (all in the engine's header): continuation is resolved geometrically as round(360/net turn) **in degrees** rather than by collision detection (so `evaluate(code, unitDegrees)` must be passed the family's angular unit, or every closed contour gets traced several times over); `⊛` is cycling elementwise addition onto absolute angles; the vanishing sign's exception clause is not modelled; the auditory half of the paper (Table 2) is out of scope. `describeExtra()` reports the structural information load I, the vocabulary the code actually uses, and the code in the paper's notation. **The paper states the value of nearly every rule as a worked example, and all of them are asserted verbatim in `tests/run.js`** — that suite is the real specification. |
| `SITCode3DIndividual` | plain-data Leeuwenberg code tree + `family` | ThreeD | ("Leeuwenberg Code 3D" in the UI.) `SITCodeIndividual` subclass; the paper's 3D half. Its whole extra mechanism is the **outerproduct** indicator `⟨v⟩`: the turtle carries a direction `d` plus the normal `n` of the plane of the last two segments, a plain angle rotates `d` about `n` (staying in plane, the 2D case) and `⟨v⟩` rotates it about `d × n` (leaving the plane, right-hand rule), after which `d × n` becomes the new normal. `⟨0⟩`/`⟨180⟩` are collinear continuations and must **not** roll the plane, or every straight run inside an out-of-plane branch corkscrews (regression-tested). Everything else — every unit, operation, indicator, combinatory rule — is the same algebra on the same value stream, which is exactly the paper's point. `⦃…⦄` over `⟨q⟩,⦃…⦄` (a closed contour carrying a tipped closed contour at each node) is the paper's cube, Table 1 shape S. **Surfaces, not just rods.** The paper's 3D figures are mostly solids — generalised cylinders, cones, vases with extrusions and hairs (Table 1, S-3 and U-1…W) — and its construction for them is parallel structure: a closed contour with a profile at each node. Because a parallel structure generates its branch code *once* and replicates it, the copies are structurally identical, so the branch family is already a regular grid of points; `SITLanguage.interpret3D` therefore returns those grids (`families`, grouped by reference identity of the shared item stream) alongside the drawn `segments`, and a **`skin` gene per parallel row** decides whether a family is lofted into a surface (closing the loop when the trunk closed — a solid of revolution) or left as rods. Both readings are in the paper: skinned is the vessel, unskinned is the hairs and struts the same figures carry. The `lathe` generator finish assembles the classic case directly, and its tip must be **two** quarter-turns `2·⟨q⟩`, not one — after a single ⟨90⟩ the reference-plane normal points along the radius, so plain angles would still bend *tangentially* (a fin); the second turns out along the radius and leaves the normal tangential, after which plain angles bend in the radial-vertical plane, which is what a lathe profile is. Remaining contour polylines are swept into parallel-transport tubes; both go into one mesh for the shared Three.js scene, with a 2D orthographic fallback, and `generate3DPoints()` gives STL export free. A headless dev tool previews either type: `node scripts/sit-preview.js [2d|3d|solid] [out.png] [--n 25] [--size 200]` — `solid` flat-shades the real mesh, and is the only way to check a skinned family without a browser. |
