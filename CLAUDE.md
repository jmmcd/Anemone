...
Any individual type can offer **direct manipulation of its rendered phenotype**, with each edit written back into the heritable genome — so a user's intervention becomes genetic material and evolution continues from it, rather than it being a one-off touch-up discarded at the next generation. The protocol is **not** grid-specific: the step sequencers use the default grid session, and `CatIndividual` is the worked example of a type supplying its own gesture (click the leg band in the zoom to cycle the gait, drag it left/right to scrub the stride rate — both written back with `setGene`).
...
- **A type with a non-grid phenotype overrides `beginEditSession` entirely.** `CatIndividual` is the reference: two gestures in the leg band below the body, separated by whether the pointer moved (the same click-vs-drag idiom as the grid session) — a click cycles the `gait` gene, a horizontal drag scrubs `stride`. It hit-tests a *region*, not a limb, because the legs are animating and a target you have to chase is a bad target. Both genes are drawn with an explicit `{ name }` in the generator, since `setGene` needs an addressable trace entry and a structural name is a source-position path that moves whenever the file is edited.
