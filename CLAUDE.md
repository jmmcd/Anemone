...
| `services/` | app-level `window.*` singletons: `Palette`, `Photo`, `AudioClip`, `ExpressionCompiler`, `MIDISync`, `OSCInput`, `SITLanguage`, `SITAnalysis` |
| `ui/` | drawer panels (`*ControlUI.js`, `MIDISyncUI`, `OSCInputUI`, `SITAnalysisUI`, `CodeEditorUI`), `PerformanceControls.js`, and the two **app-wide** panels the framework mounts itself rather than attaching per type: `HelpOverlayUI` (the `?` overlay) and `EvolutionControlsUI` (the Evolution panel) |
...
| `scripts/` | headless dev tools (preview renderers, `sit-music.js`, EEG sender, PTO repro) and `git-hooks/` |
...
**The auditory half (`interpretMusic`).** The paper's title is "…for Visual **and Auditory** Patterns", and its claim is that one algebra covers both. `interpretMusic(items)` is that reading, sitting beside `interpret2D`/`interpret3D`: **nothing in `evaluate` changes** — a value is a pitch interval, one per elementary grain of *time*, exactly as the visual half reads it as an angle per grain of contour. `SITMusicIndividual` is its client. The correspondence is exact rather than analogical because **angle ↔ pitch class and full turn ↔ octave** (the chroma circle), so the engine's 360°-based closure arithmetic *is* pitch arithmetic when `evaluate` gets `unit = 360/N`, and the rotational family N becomes the scale (N notes to the octave). What falls out: `n·(0)` is a note held n grains (length is not a primitive in either reading), `‾ ‾` is a rest, `| |` is pitch from the tonic, `∫` is intervals→absolute pitches, `±` is inversion, **`R` is the paper's symmetry — an arch, i.e. the phrase plus its retrograde *inversion*, since R reverses intervals while a true retrograde reverses *and* negates them** (both symmetry operations append rather than transform, so a bare retrograde is not something this algebra says), `+` is transposition (and `k·{X} + (0,c…)` is the classical **sequence**, via the paper's own "a constant lands on a chunk's head" rule), a parallel continuation is a **chord** stacked by one interval, a parallel structure is **polyphony**, `‖` is a tonal rather than real answer, `⊛` is a melody over a **cantus firmus**, and `⦃ ⦄` repeats until the melody returns to its starting pitch class. The one liberty is `⟨ ⟩`: the paper's outerproduct is spatial, and here it is the **time** dimension (augmentation/diminution), because for a stream of pitches that is the orthogonal one. **`_countValues` must stay memoised by array identity** — a parallel structure attaches ONE evaluated branch at every node, so the item stream is a DAG, and walking it as a tree costs the product of the row lengths (seconds, before a single note or mark is produced; regression-tested).

### Reverse map: music → code (`SITAnalysis.js`)
`window.SITAnalysis` runs SIT in the direction it was actually built for. Everywhere else Anemone is *generative* (write a code, decode it, let the minimum principle act as a prior); SIT is natively an **analysis** theory — the shortest code is the claim about what a listener hears. `analyse(notes)` quantises a melody to grains, picks the smallest scale that holds every pitch (`chooseKey` — itself an application of the minimum principle), turns it into the atom stream `interpretMusic` reads (interval at an onset, 0 while held, 0-vanished for a rest), and searches for a minimal-load code by a **memoised dynamic program over substrings** using six rules: literal, serial split, `k·{X}` iteration, `R` mirror, `±` inversion, and the transposed repeat `k·{X} + (0,d…)`. Continuation, parallel structure, `⊛` and `∫` are deliberately **not** searched (a continuation's count is resolved geometrically, so it can't be guaranteed exact), which makes a result an *upper bound* on I, never a claim of the true minimum. **Exactness is guaranteed and asserted**: `verify` replays the code through the app's own `SITLanguage.evaluate` and compares streams for every built-in melody. Two contracts matter: `tidy` must flatten a whole border spine *before* re-grouping (level-by-level flattening re-nests the groups and leaves the tree as deep as the DP's binary chain, which then can't be seeded), and `SIT_ANALYSIS_MAX_SEQ` mirrors the generator's border arity. A repeated note is articulated by clipping one grain off the note before it and vanishing it — otherwise successive grains at one pitch are *one held note*, which is the same identity the visual half has (collinear grains are one line). Built-in melodies live here as a one-line `PITCH:GRAINS` notation; `MidiExport.parseSMF` (the reader beside the writer) supplies real `.mid` files. Headless QA: `node scripts/sit-music.js analyse`.
...
| `AnimatedPatternIndividual` | `{ expression, periodSeconds }` | Canvas2D (animated) | Like `PatternGrammar` but the grammar adds time terminals (`t`, `sin/cos(2πnt)`); a per-canvas `requestAnimationFrame` loop animates each tile, at an evolved period. (Hidden in the UI selector — too heavy for mobile/old hardware.) |
...
| `SITMusicIndividual` | plain-data Leeuwenberg code tree + `family`/`tonic`/`bpm` | MIDI / audio | ("Leeuwenberg Code Music" in the UI.) `SITCodeIndividual` subclass; the paper's **auditory** half, and the point is that nothing in the algebra changed — see `interpretMusic` above for the full mapping, and the file header for why it works (angle ↔ pitch class, turn ↔ octave, so the family gene is the **scale**: 3 augmented, 4 dim7, 5 pentatonic, 6 whole-tone, 7 diatonic, 8 octatonic, 12 chromatic). Rides the shared step-sequencer path (`playSequenced`: live MIDI else a synthesised loop), so it drives Logic/GarageBand over IAC and exports `.mid` free; the tile is a piano roll, one colour per voice. **Where it parts from the figure types:** the interval literal is a *weighted pool* spanning the octave, not a range capped at half the family — that cap is an angular argument (a turn past 180° is a turn the other way) and pitch is not circular, so it would forbid even a fifth. Two genes exist only to make the reverse map work — the **depth budget** and the **border arity**, both drawn from pools skewed small so random codes stay shallow while an analysed melody can reach the depth it needs. **`validate()` also rejects clusters** (polyphony > 8): the language is not at fault, but not every legal code is worth hearing. `SITMusicIndividual.fromCode(code)` is the seeding path — see below. Headless QA: `node scripts/sit-music.js [analyse]`. |
...
### Importing external material as a genome (inverse PTO)

The same "it only counts if it goes through the representation" rule applies to material that comes from *outside* — a melody analysed by `SITAnalysis`. A PTO genome is a trace of `rnd` decisions, so an imported code can only become a genome by being **generated**: `SITMusicIndividual.fromCode(code)` computes, with `leeEncodeMusic`, the decisions the generator would have had to make to produce that code, and drives the generator down that path by patching `Math.random` with a queue of u-values for the duration of one `generateRandom()`. That works because every PTO distribution samples with exactly one `Math.random()` call and inverts exactly (uniform → linear; `randint`/`choice` → the bucket's **midpoint**, so the gene lands mid-bucket and mutation creeps it to a neighbour just as for an evolved genome). PTO's tracer then names and records the entries itself — no name prediction, no reaching inside the tracer.

The encoder is unavoidably a **mirror of the generator** (same order of draws, same thresholds, same pools). That coupling is managed, not wished away: the thresholds and pools are top-level consts (`LEE_MUSIC_KINDS`, `LEE_MUSIC_DURS`, `LEE_MUSIC_OPS`, `leeIntervalPool`, `LEE_MUSIC_DEPTHS`, `LEE_MUSIC_SEQ_POOL`) that both sides read, and `fromCode` **verifies by replay** — it expresses the resulting genome and compares evaluated item streams, returning `null` if anything differs. So a generator edit, or a code too deep to express, degrades to "analysis only, no seed" rather than to a near-miss. If you change the generator's draws, change the encoder in the same commit; the melody tests fail loudly if you don't. Two ordering traps are already paid for: the generator draws a leaf's **run length before its interval**, and a `&&`-guarded `rnd` call at the root is *not made at all* — the mirror has to match both.
...
- `SITAnalysisUI` — the **Leeuwenberg Analysis** panel, attached when an individual returns `true` from `usesSITAnalysis()` (only `SITMusicIndividual`, the only type whose generator can be run backwards). Analyse a built-in melody or a dropped `.mid`, read its code / structural information load / compression, hear the reconstruction, and **seed the population** from it.
