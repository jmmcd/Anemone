...
- **Glow / bloom**: `Canvas2DModality.bloom(imageData, { radius, strength, background })` adds a soft palette-coloured halo over the finished ImageData; used by `PolarCurveIndividual` and `AnemoneIndividual` (the latter reads as bioluminescent underwater life against its black background). Scale `radius` by `s` (above) so the glow matches at zoom.
