// WonkyGuysIndividual — the *original* Robot draw, kept alive as its own
// type after RobotIndividual was redesigned into a coherent body-relative figure
// (commit "Redesign RobotIndividual"). Here the geometry genes are independent
// canvas coordinates rather than body-relative, so mutation happily drifts the
// head, limbs, and face apart into the weird, deformed little characters that
// gave this type its charm. Same 43-gene vector and draw code as the pre-redesign
// Robot; only the module-level names and the class name differ (both scripts load
// as plain globals, so the consts must not collide with RobotIndividual's).

// PTO generator: builds the 43-gene parameter vector, each gene drawn from its
// own range. PTORepresentation's default 'fine' mutation is Gaussian creep within
// each gene's range.
const wonkyGenerator = (rnd) => [
    // Head (4)
...
const wonkyRepresentation = new PTORepresentation(wonkyGenerator);

// The whole character lives in this draw function — not the flat 43-float
...
const wonkyDraw = new Editable(function (self, ctx, width, height) {
    // Clear canvas
...
class WonkyGuysIndividual extends Individual {
    constructor(genome = null) {
        super('SKIP_GENOME_GENERATION');
        this.representation = wonkyRepresentation;
    ...
    editableSections() {
        return [
            Individual.functionSection('Draw', wonkyDraw),
    ...
    visualize(canvas) {
        wonkyDraw.value(this, canvas.getContext('2d'), canvas.width, canvas.height);
    }
